package com.arcticraft.gui.button;
...
public class AC_GuiTraderButton extends GuiButton
{
	...
	private GuiTraderEskimo traderGui;
	private static RenderItem itemRenderer = new RenderItem();

	public AC_GuiTraderButton(int ID, int x, int y, AC_EskimoTrade trd, GuiTraderEskimo gui)
	{
		super(ID, x, y, 18, 18, "");
		this.trade = trd;
		this.traderGui = gui;
	...
	public void drawButton(Minecraft mc, int pointX, int pointY)
	{
		if(this.func_146115_a())
		{
			...
			this.field_146123_n = pointX >= this.xPosition && pointY >= this.yPosition && pointX < this.xPosition + this.width && pointY < this.yPosition + this.height;
	...
	protected void drawName(ItemStack itemstack, boolean gems, int par2, int par3, Minecraft mc)
	{
		...
		if(! list.isEmpty())
		{
			...
			for(int var13 = 0; var13 < list.size(); ++var13)
			{
				...
				if(var13 == 0)
				{
					var14 = "\u00a7" + Integer.toHexString(itemstack.getRarity().rarityColor.hashCode()) + var14;
				}
